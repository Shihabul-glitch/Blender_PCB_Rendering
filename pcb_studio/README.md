...
Install the current **v2.3.2** ZIP from the repository's `Releases/` directory.
Build it from the repository root with Blender's extension builder:
...
## Product Photography Floors

Studio > Floor / Stage offers Standard, Infinite Studio, Shadow Catcher, Custom,
and No Floor modes, with Simple and Advanced controls. Custom floors restore their
original material and visibility when released. Ground Product and Reset Ground
provide reversible vertical placement; Auto Ground follows floor and gap changes.
Native shadow catching requires Cycles, and its transparency setting is restored
when leaving the mode or resetting the studio. Shadow strength and softness are
controlled through studio lighting. Cast Shadows controls Cycles ray visibility.
...
[Product Presentation](PRODUCT_PRESENTATION.md), including animation safety,
object classification, and tested limitations.
